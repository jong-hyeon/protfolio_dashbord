...
export function AssetTable({ assets }: { assets: Asset[] }) {
  ...
  return <Table><TableHeader><TableRow><TableHead>자산</TableHead><TableHead>계좌</TableHead><TableHead>자산군</TableHead><TableHead className="text-right">수량</TableHead><TableHead className="text-right">현재가</TableHead><TableHead className="text-right">평가금액</TableHead></TableRow></TableHeader><TableBody>{assets.map((asset) => <TableRow key={asset.id}><TableCell className="font-medium">{asset.name}</TableCell><TableCell>{accountLabels[asset.accountType]}</TableCell><TableCell>{classLabels[asset.assetClass]}</TableCell><TableCell className="text-right">{asset.quantity.toLocaleString("ko-KR")}</TableCell><TableCell className="text-right">{formatKrw(asset.currentPrice)}</TableCell><TableCell className="text-right font-medium">{formatKrw(calculateAssetValue(asset))}</TableCell></TableRow>)}</TableBody></Table>;
}
